 ...
 * - Si recibe `desde` (YYYY-MM-DD), hace incremental. Si no, completo.
 */
export async function syncCatalog(desde?: string): Promise<SyncResult> {
  const start = Date.now();
  const productRepo = AppDataSource.getRepository(Product);
  const configRepo = AppDataSource.getRepository(Config);

  const articulos = desde
    ? await getArticulosDesde(desde)
    : await getArticulos(false);
  ...
  // Guardamos la fecha de esta corrida para el próximo incremental
  let config = await configRepo.findOneBy({ id: 1 });
  if (!config) config = configRepo.create({ id: 1 });
  config.lastAlephSync = new Date();
